...
function extractUrlObject(pattern, asValue, normalizeAsPathValue) {
  ...
  for (let i = 0; i < patternSegments.length; i += 1) {
    ...
    const dynamicSegment = parseDynamicRouteSegment(segment);

    if (dynamicSegment && dynamicSegment.repeat) {
      return null;
    }

    if (dynamicSegment) {
      params[dynamicSegment.name] = asSegment;
      hasParams = true;
      continue;
    }
...
function formatParameterList(parameters) {
  const quoted = parameters.map((parameter) => `'${parameter}'`);
...
function createNavigationReporter(context, ruleContext, options = {}) {
  ...
  function reportMissingQueryParams({
    node,
    pathname,
    missingParams,
    method,
    ...
    if (!routeSet.has(normalizedPathname)) {
      return false;
    }

    if (hasUnknownAs || asWasReported) {
      return false;
    }
    ...
    if (typeof asValue === 'string') {
      ...
      if (!asCandidate || !asCandidate.startsWithSlash) {
        return false;
      }

      const normalizedAs = normalizeTrailingSlash(
        normalizeAsPathValue(asValue)
      );

      if (patternToRegex(normalizedPathname).test(normalizedAs)) {
        return false;
      }
    }

    const isPlural = missingParams.length > 1;

    context.report({
      node,
      messageId: isPlural
        ? messages.missingQueryParams
        : messages.missingQueryParam,
      data: {
        method,
        pathname,
        param: missingParams[0],
        params: formatParameterList(missingParams),
      },
    });
    return true;
  }
