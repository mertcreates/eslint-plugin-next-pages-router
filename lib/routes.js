...
function parseDynamicRouteSegment(segment) {
...
function getDynamicRouteParameters(pattern) {
  return pattern
    .split('/')
    .filter(Boolean)
...
function patternToRegex(pattern) {
  ...
  const segments = pattern
    ...
    .filter(Boolean)
    .map((seg) => {
      const dynamicSegment = parseDynamicRouteSegment(seg);

      if (dynamicSegment && dynamicSegment.optional) {
        return '(?:/.*)?';
      }
...
function buildDynamicMatcherIndex(dynamicMatchers) {
  ...
  for (const [index, matcher] of (dynamicMatchers || []).entries()) {
    ...
    const hasCatchAll = segments.some((segment) => {
      ...
      return dynamicSegment && dynamicSegment.repeat;
    });
